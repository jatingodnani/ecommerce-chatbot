#!/usr/bin/env python3
"""
Data loading through FastAPI endpoints - using the same connection as the working server
"""
import pandas as pd
import os
...
class AsyncDataLoader:
    ...
    async def connect(self):
        """Connect to MongoDB using async Motor client (same as FastAPI)"""
        try:
    ...
    async def load_distribution_centers(self):
    ...
    async def load_inventory_items(self):
        ...
        # Read in smaller chunks to handle large files
        chunk_size = 2000
        total_inserted = 0
        chunk_num = 0
    ...
    async def load_all_data(self):
        ...
        logger.info("🚀 Starting async data loading process...")
        ...
        try:
            success = True
            success &= await self.load_distribution_centers()
            success &= await self.load_products()
            success &= await self.load_users()
            success &= await self.load_orders()
            success &= await self.load_inventory_items()
            success &= await self.load_order_items()
